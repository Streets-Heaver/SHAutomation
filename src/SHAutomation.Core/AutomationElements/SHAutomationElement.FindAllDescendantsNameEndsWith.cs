...
namespace SHAutomation.Core.AutomationElements
{
    ...
    {
        public ISHAutomationElement[] FindAllDescendantsNameEndsWith(string name, int timeout = 20000, bool waitUntilExists = true)
        {
            List<ISHAutomationElement> elements = new List<ISHAutomationElement>();
            bool getElements(bool shouldExist)
            {
                if (!elements.Any())
                    elements = FindAllDescendants(timeout: 0).Where(x => x.FrameworkAutomationElement != null && x.SupportsName && !string.IsNullOrEmpty(x.Name) && x.Name.EndsWith(name)).ToList();
